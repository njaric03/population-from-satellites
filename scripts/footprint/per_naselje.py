...
Po naselju se racuna 11 atributa u tri grupe:

* kolicina    – broj zgrada, krovna povrsina, zapremina-proxy (povrsina*spratnost)
* oblik       – prosecna i medijalna povrsina zgrade, kompaktnost (4*pi*A/O^2)
* raspored    – rastojanje do najblize zgrade i broj suseda u 50 m; razdvaja
                zbijeno selo od rastrkanog sa istim brojem zgrada, sto sam
                broj zgrada ne vidi
* visina      – prosecna spratnost i udeo zgrada sa poznatom visinom (Overture
                je popunjava neravnomerno, pa je udeo sam po sebi signal)
...
for k in okruzi:
    ...
    try:
        ...
        b["visina_poznata"] = (fl.notna() | h.notna()).astype("float32")
        ...
        kol = ["ba", "vol", "compact", "spratnost", "visina_poznata", "nn_dist", "n_50m"]
        ...
        agg = j.groupby("naselje_maticni_broj").agg(
            ...
            mean_floors=("spratnost", "mean"),
            height_known=("visina_poznata", "mean"),
...
ATRIBUTI = ["n_buildings", "roof_area_m2", "vol_proxy", "mean_bsize", "median_bsize",
            "mean_compact", "mean_floors", "height_known", "mean_nn_dist",
...
# izvedeni odnosi (gustine po povrsini naselja) — ne traze nova preuzimanja
...
m["vol_density"] = m["vol_proxy"] / m["area_km2"].clip(lower=1e-6)
...
for c in [*ATRIBUTI, "building_density", "built_fraction", "vol_density"]:
